...
def _read(db_path: Path) -> list[tuple] | None:
    ...
    try:
        return conn.execute(
            ...
            "incremental_revenue, is_money_losing "
...
def build_promo_roi(ws: Worksheet, db_path: Path, built_date=None) -> None:
    rows = _read(db_path)

    ws.sheet_view.showGridLines = False

    col_widths = [3, 14, 14, 20, 12, 12, 14, 14, 14, 14, 12, 16, 14]
    ...
    total = len(rows)
    money_losing = sum(1 for r in rows if r[12] == 1)
    ...
    headers = [
        ...
        "Promo Weeks", "Incremental Rev", "Money-Losing",
    ]
    header_row = row
    ...
    def _sort_key(r):
        ...
        return (0 if r[7] else 1, 0 if r[12] == 1 else 1, -(r[5] or 0))
    ...
    for i, row_data in enumerate(sorted_rows):
        (promo_id, sku_id, retailer, start_week, end_week, promo_cost, promo_type,
         ...
         incr_rev, is_money_losing) = row_data
        ...
        if is_money_losing is None:
            ws.cell(row=r, column=14, value="N/A").alignment = ALIGN_CENTER
        else:
            ws.cell(row=r, column=14, value="Yes" if is_money_losing else "No").alignment = ALIGN_CENTER

    table_end = header_row + len(sorted_rows)

    tbl = Table(displayName="tbl_PromoROI", ref=f"B{header_row}:N{table_end}")
    tbl.tableStyleInfo = TABLE_STYLE
    ws.add_table(tbl)

    ml_range = f"N{header_row + 1}:N{table_end}"
    ...
    ws.merge_cells(f"B{note_row}:M{note_row}")
    note = ws.cell(
