...
- **Schema details confirmed** (from `trade-spend-data-diagnostic/HANDOFF.md`): `scan_data` uses `week_ending` (not `week_start`), revenue = `dollars_sold`, not `units × price`. `sku_costs` has per-channel columns (`trade_spend_pct_walmart`, `trade_spend_pct_costco`, `trade_spend_pct_whole_foods`, `trade_spend_pct_unfi`, `trade_spend_pct_dtc`, `trade_spend_pct_kehe`, `trade_spend_pct_regional`). `deductions.retailer_id` uses slugs (`walmart`, `costco`, etc.). **⚠ `promotions.retailer_id` uses a DIFFERENT format: `RET-WALMART`, `RET-COSTCO`, etc.** Any SQL joining `raw.promotions` to `raw.retailer_deductions` must use a slug_map CTE — direct join on `retailer_id` silently returns 0 rows. See `docs/solutions/logic-errors/retailer-id-format-mismatch-join-produces-wrong-leakage-2026-06-01.md`.
- **Actual data numbers** (from live Fly.io Postgres, corrected 2026-06-01 after fixing Move 3 slug_map bug): 1,521 total leakage instances / $144,320 total leakage (all unauthorized deductions). Double-funded and rate-discrepancy detection returned 0 instances — the correct result for the Cinderhaven dataset. Ghost promos: correct count after fix. Earlier figure of $235,760 was inflated due to the retailer_id format mismatch bug. Postgres dataset is ~4.5× larger than the SQLite snapshot in `TRADE_SPEND_VERIFICATION.md` — Postgres is SSOT per CLAUDE.md.
