...
- **Do not:** Re-hardcode a week or month count in any rendered surface. If a span must appear, read it from the pipeline output and omit it when unavailable.
- **Follow-up landed:** The Excel workbook (the other rendered surface) now derives the same spans via `workbook/windows.py` (`read_trailing_weeks` / `read_trailing_months`), consumed by `tab_net_revenue.py`, `tab_efficiency.py`, `tab_accrual.py` (month count from `len(rows)`), and `tab_summary.py`. Locked by the window-label tests in `tests/test_workbook.py`. Rendered text is byte-identical for canonical data (52 weeks / 12 months) and the workbook is generated on the fly (no byte-golden), so nothing else moved.
...
- **Also resolved:** The same defect class in the Excel workbook (`workbook/tab_net_revenue.py`, `tab_efficiency.py`, `tab_accrual.py`, `tab_summary.py`) was fixed in the same session via `workbook/windows.py` + `tests/test_workbook.py` window-label tests. See the resolution entry's "Follow-up landed" note under Architecture & Pipeline.
